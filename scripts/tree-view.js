...
(function() {
    ...
    */
    var UI_TREE_VIEW = ui.TreeView =
        inheritsControl(
            ...
            function(el, options) {
                // if(!this.isRender) {
                //     el.innerHTML = options.datasource ? this.getMainHtml(options.datasource) : '';
                //     this.isRender = 1;
                // }
                ...
                if (o && o.tagName == 'LABEL') {
                    ...
                    addClass(el, options.current = options.primary + '-leaf');
                }
            },
            function(el, options) {
                //console.debug(el.innerHTML, el.className);
    ...
    */
    function UI_TREE_VIEW_FLUSH(control) {
        control.setClass(
            control.getPrimary() + (control._aChildren.length ? control._bCollapsed ? '-collapsed' : '-expanded' : '-leaf')
        );
    }
    ...
    */
    ...
    UI_TREE_VIEW_CLASS.getMainHtml = function(datasource) {
        function createNode(node) {
            if (!node.children) {
                return node.text;
            }
            var html = [];
            html.push('<label>' + node.text + '</label>');
            for (var i = 0, len = node.children.length; i < len; i++) {
                html.push('<div>' + createNode(node.children[i]) + '</div>');
            }
            return html.join('');
        }
        return createNode(datasource);
    };
    //{/if}//
    //{if 0}//
})();

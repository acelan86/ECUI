...
* bug:日历在form上时zindex应该比form高，增加一个获取form zindex方法
*/
(function() {
    ...
    */
    var UI_CALENDAR = ui.Calendar =
        ...
        inheritsControl(
            ...
            function(el, options) {
                ...
                el = el[1];
                this._uMonthView = $fastCreate(monthViewClass, el, this, { range: options.range });
            }
        ),
    ...
    UI_CALENDAR_CLASS.$activate = function(event) {
        ...
        if (!layer.isShow()) {
            ...
            con = getView();
            triggerEvent(this, 'showbefore');
            layer.show();
            var layerHeight = layer.getMain().offsetHeight;
            layer.setPosition(
                pos.left + layer.getWidth() <= con.right ? pos.left : con.right - layer.getWidth() > 0 ? con.right - layer.getWidth() : 0,
                posTop + layerHeight <= document.documentElement.clientHeight ? posTop : pos.top - layerHeight > 0 ? pos.top - layerHeight : 0
            );
            setFocused(layer);
        }
    };
    ...
    UI_CALENDAR_CANCEL_CLASS.$click = function() {
        ...
        if (triggerEvent(par, 'change', ["-1", "-1"])) {
            par.setDate(null);
        }
    };
    ...
    UI_CALENDAR_LAYER_CLASS.$blur = function(event) {
        this.hide();
    };
    ...
    UI_CALENDAR_LAYER_CLASS.ondateclick = function(event, date) {
        var par = this.getParent();
        if (triggerEvent(par, 'dateset', event, [date])) {
            par.setDate(date);
        }
        this.hide();
    };
    ...
    UI_CALENDAR_LAYER_SELECT_CLASS.onchange = function() {
        UI_CALENDAR_MONTHVIEW_FLUSH(this.getParent());
    };
    ...
    */
    ...
    UI_MULTI_CALENDAR_CLASS.setRange = function(begin, end) {
        this._uLayer._uBeginCal.setRange(begin, end);
        this._uLayer._uEndCal.setRange(begin, end);
        this.range = {
            begin: begin,
            end: end
        }
    ...
    UI_MULTI_CALENDAR_LAY_CLASS.onchange = function() {
        var par = this.getParent(),
    ...
    UI_MULTI_CALENDAR_LAY_CLASS.$setDate = function(date, type) {
        ...
        if (type == 'begin') {
            this._uEndCal.setRange(date, range ? range.end : undefined);
        }
        else {
            this._uBeginCal.setRange(range ? range.begin : undefined, date);
        }
    };
